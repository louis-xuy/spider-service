...
def get_as_cp():
    ...
    # print e
    i = hashlib.md5(str(int(
        now))).hexdigest().upper()  # hashlib.md5().hexdigest()创建hash对象并返回16进制结果
    if len(e) != 8:
...
def get_detail(group_id):
    ...
    # print(response.text)
    text = response.text
    content = text[text.find("content: '") +
                   len("content: '"):text.find("groupId: '")]
...
def inter():
    ...
    while True:
        ...
        for u in users:
            ...
            for s in ss:
                print(s)
                if s['publish_time'] > ts[0] and s['publish_time'] <= ts[
                        1] and not s['has_video']:
...
class TouTiaoSpider(scrapy.Spider):
    ...
    def parse_list(self, response):
        ...
        headers = {
            'user-agent':
            "Mozilla/5.0 (Linux; Android 4.4.4; MuMu Build/V417IR) \
                AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/33.0.0.0 Mobile Safari/537.36 JsSdk/2 NewsArticle/6.3.1 NetType/wifi",
            'cache-control': "no-cache"
        }
    ...
    def parse_news(self, response):
        ...
        if sel.xpath(
            ...
            title = sel.xpath(
                ...
                '//*[@id="C-Main-Article-QQ"]/div/div[1]/div[1]/div[1]/h1/text()'
        ):
            title = sel.xpath(
                '//*[@id="C-Main-Article-QQ"]/div/div[1]/div[1]/div[1]/h1/text()'
            ).extract()[0]
        ...
        url = response.url
        if sel.xpath(
                '//*[@id="Main-Article-QQ"]/div/div[1]/div[1]/div[1]/div/div[1]/span[3]/text()'
        ):
            time_ = sel.xpath(
                '//*[@id="Main-Article-QQ"]/div/div[1]/div[1]/div[1]/div/div[1]/span[3]/text()'
            ).extract()[0]
        ...
        if response.xpath(
                '//*[@id="Main-Article-QQ"]/div/div[1]/div[2]/script[2]/text()'
        ):
            cmt = response.xpath(
                '//*[@id="Main-Article-QQ"]/div/div[1]/div[2]/script[2]/text()'
            ).extract()[0]
            if re.findall(r'cmt_id = (\d*);', cmt):
                ...
                comment_url = 'http://coral.qq.com/article/{}/comment?commentid=0&reqnum=1&tag=&callback=mainComment&_=1389623278900'.format(
                    cmt_id)
