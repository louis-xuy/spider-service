...
class FileException(Exception):
    ...
    def __str__(self):  #####for usage: print e
        print(
            self.style.ERROR("ERROR(FileException): %s" %
...
class FilePipeline(MediaPipeline):
    ...
    def from_settings(cls, settings):
        ...
        cls.ATTACHMENT_FILENAME_UTF8_DOMAIN = settings.get(
            'ATTACHMENT_FILENAME_UTF8_DOMAIN', [])
    ...
    def media_downloaded(self, response, request, info):
        ...
        if response.status != 200:
            log.msg(
                ...
                request=request,
                referer=referer)
            raise FileException(request.url,
        ...
        if self.is_valid_content_type(response):
            raise FileException(request.url,
                                '%s: invalid-content_type' % (request.url, ))
        ...
        try:
            ...
            checksum = self.store.persist_file(key, response.body, info,
                                               filename)
    ...
    def media_failed(self, failure, request, info):
        ...
        raise FileException(request.url,
                            '%s: Error downloading' % (request.url, ))
    ...
    def media_to_download(self, request, info):
        def _onsuccess(result):
            ...
            log.msg(
                format=
                '%(medianame)s (uptodate): Downloaded %(medianame)s from %(request)s referred in <%(referer)s>',
                level=log.DEBUG,
                spider=info.spider,
                medianame=self.MEDIA_NAME,
                request=request,
    ...
    def inc_stats(self, spider, status):
        spider.crawler.stats.inc_value('%s_file_count' % (self.MEDIA_NAME, ),
                                        spider=spider)
        spider.crawler.stats.inc_value('%s_file_status_count/%s' % (
            self.MEDIA_NAME, status),
                                        spider=spider)
    ...
    def get_file_name(self, request, response):
        ...
        if filename:
            ...
            if urlparse(request.url
                        ).netloc in self.ATTACHMENT_FILENAME_UTF8_DOMAIN:
            ...
            if os.path.splitext(guessname)[1].lower() in self.FILE_EXTENTION:
                if urlparse(request.url).netloc in self.URL_GBK_DOMAIN:
                    filename = urllib.unquote(guessname).decode("gbk").encode(
                        "utf-8")
                else:
