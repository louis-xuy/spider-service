...
import json
import sys
from scripts.toutiao import db
...
def d(user_id):
    ...
    querystring = {"user_id": user_id, "iid": "31047425023", "device_id": "51425358841", "ac": "wifi",
                   "channel": "tengxun", "aid": "13", "app_name": "news_article", "version_code": "631",
                   "version_name": "6.3.1", "device_platform": "android",
                   "ab_version": "297979,313416,319436,317498,295827,325046,323882,239097,324283,170988,325961,320218,325198,281392,297058,276203,286212,313219,328615,329603,329358,322321,327537,326536,321981,328095,328670,324007,324795,317077,324571,280773,330104,319960,326730,317208,322280,214069,31643,318434,207253,266310,321519,258356,247847,281298,328218,320995,325618,328376,330275,323429,287591,288418,260650,326188,324614,271178,326588,326524,326532",
                   ...
                   "update_version_code": "6310", "_rticket": "1524463139363", "plugin": "256"}
    ...
    data = jj['data']
    r_following_cnt = 0
    print (type(LongField(user_id)))
    print ('--',db.User.objects(ID=user_id).count())
    for user in db.User.objects(ID__gt>1).all():
        print (user.ID)
        print ('----')
...
def t(user_id, page, cursor):
    ...
    querystring = {"count": "20", "offset": page * 20, "cursor": cursor, "user_id": user_id, "plugin_info": "0",
                   "iid": "31047425023", "device_id": "51425358841", "ac": "wifi", "channel": "tengxun", "aid": "13",
                   ...
                   "os_version": "4.4.4", "uuid": "008796762094657", "openudid": "b7215ea70ca32066",
                   "manifest_version_code": "631", "resolution": "1280*720", "dpi": "240",
                   "update_version_code": "6310", "_rticket": "1524461579082", "plugin": "256"}
    ...
    jj = json.loads(response.text)
    for fo in jj['data']:
        if fo['is_verified'] == 1 and fo['user_id'] in g_id_cache:
            user = db.User()
            user.ID = fo['user_id']
            user.NAME = fo['name']
            user.ST = 0
            db.User.objects.insert(user)
            try:
                g_fc += 1
                print (g_dc, g_fc, 'Found', user.ID, fo['name'])
                g_id_cache[user.ID] = 1
            except Exception:
                db.User.objects.rollback()
...
def routine(start):
    last_cursor = 0
    c = 0
    for user in db.User.objects(db.User.ST == 0).limit(10):
        fc = d(user.ID)
        c += 1
